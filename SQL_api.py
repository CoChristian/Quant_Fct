...
class SQL_API ():
    ...
    dup_data_engine = create_engine( "mysql+pymysql://develop:haikuan_2025@localhost/dup_data")
    ...
    def __init__(self,read_engine = None, save_engine = None, dup_data_engine = dup_data_engine):
        ...
        for engine_name,engine in zip(['read_engine','save_engine','dup_data_engine'],[read_engine,save_engine,dup_data_engine]):
    ...
    def file_exists(self, table_name): 
        ...
        # else:
        #     return False
        inspector = inspect(self.save_engine)
        # 如果表在特定 schema 中，可以传入 schema 参数，例如：
        # return inspector.has_table(table_name, schema='your_schema')
    ...
    def delete_old_data_and_insert_new_data(self, df, to_table):
        """
        删除旧的重复数据，插入新的数据
        """
        if df.empty:
        ...
        new_trade_dates = list(df['trade_date'].unique())

        schema = self.save_engine.url.database
        from pymysql import connect
        db = connect(
            host="localhost",
            user='develop',
            password="haikuan_2025",
            database=schema,
            port=3306,
            charset="utf8"
        )
        cur = db.cursor()
        # read strategy pre-saved position if the strategy is in the fund
        for trade_date in new_trade_dates:
            sql = "delete from {} where trade_date = {}".format(to_table, trade_date)
            print(sql)
            cur.execute(sql)
            db.commit()
        cur.close()
        db.close()
        df.to_sql(name = to_table, con=self.save_engine,index = False,if_exists = 'append')
...
#             pdb.set_trace()
        ...
        if incremental_df.empty:
            ...
            print('*'*40)
            print('new data added to {to_table}, total {num_row} rows'.format(to_table = to_table,num_row = incremental_df.shape[0]))
    
    def get_unique_index(self,tablename, min_trade_date, index_cols = ['trade_date','ts_code'],engine = 'read'):
        """
        ...
        if min_trade_date != 0 and 'trade_date' in index_cols:
            query_info =   {'method': 'select',
             'sheet_name': tablename,
             'tgt_field': {'way': 'show', 'field': index_cols},
             'conditions': [{'field': 'trade_date',
               'type': 'big_equal',
        ...
        index_df = index_df.drop_duplicates(index_cols)
        return index_df
    ...
    def get_incremnetal_index(self,from_table,
        ...
        # read in new index
        #     new_index = self.get_unique_index(from_table, index_cols=index_cols,engine = from_engine).set_index(index_cols).index
            from_table =  self.get_unique_index(from_table, min_trade_date=0, index_cols=index_cols,engine = from_engine).set_index(index_cols)
        # elif isinstance(from_table,pd.DataFrame):
        new_index = from_table.index
        if 'trade_date' in index_cols:
            min_trade_date = from_table.reset_index()['trade_date'].min()
        else:
            min_trade_date = 0
        # readin old index
        # if old table exist, read old index 
        if self.file_exists(to_table):
            old_index = self.get_unique_index(to_table, min_trade_date=min_trade_date, index_cols=index_cols,engine = to_engine).set_index(index_cols).index
            # get incremental index
            print('old index len {}'.format(len(old_index)))
    ...
    def check_duplicates(self,tablename,index_cols = ['id']):
        ...
        query_stmt = """select {index_cols},{count_stmt}
                ...
                group by {index_cols}
                having
                    {condition_stmt}""".format(count_stmt = ','.join(['count({col})'.format(col = col) for col in index_cols]),
...
def translate_condition_info_2_query(condition_info):
    ...
    if logical == "element":
        ...
        assert type_ in ['big_equal', 'big', 'less', 'less_equal', 'equal', 'not_equal', 'between', 'in', 'not_in']
        if type_ == "big_equal":
